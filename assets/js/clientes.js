...
function abrirModalEdicao(dados) {
    document.getElementById('cli_id').value = dados.id; document.getElementById('cli_codigo').value = dados.codigo_cliente || ''; document.getElementById('cli_nome').value = dados.nome || ''; document.getElementById('cli_cpf').value = dados.cpf || ''; document.getElementById('cli_end').value = dados.end || ''; document.getElementById('cli_num').value = dados.num || ''; document.getElementById('cli_quadra').value = dados.qd || ''; document.getElementById('cli_bairro').value = dados.bairro || ''; document.getElementById('cli_cond').value = dados.cond || ''; document.getElementById('cli_comp').value = dados.comp || ''; document.getElementById('cli_cid').value = dados.cid || ''; document.getElementById('cli_cep').value = dados.cep || ''; document.getElementById('cli_tel').value = dados.tel || ''; document.getElementById('cli_wpp').value = dados.wpp || ''; document.getElementById('cli_email').value = dados.email || ''; document.getElementById('cli_obs').value = dados.obs || ''; document.getElementById('cli_arq_nome').value = dados.arq_nome || ''; document.getElementById('cli_arq_wpp').value = dados.arq_wpp || ''; document.getElementById('cli_arq_email').value = dados.arq_email || '';
...
async function salvarClienteServidor(event) {
    ...
    const payload = {
        id: id, codigo_cliente: document.getElementById('cli_codigo').value, nome_contrato: document.getElementById('cli_nome').value, cpf_cnpj: document.getElementById('cli_cpf').value, telefone: document.getElementById('cli_tel').value, whatsapp: document.getElementById('cli_wpp').value, email: document.getElementById('cli_email').value, endereco: document.getElementById('cli_end').value, numero_lote: document.getElementById('cli_num').value, quadra: document.getElementById('cli_quadra').value, bairro: document.getElementById('cli_bairro').value, condominio: document.getElementById('cli_cond').value, complemento: document.getElementById('cli_comp').value, cidade: document.getElementById('cli_cid').value, cep: document.getElementById('cli_cep').value, observacao: document.getElementById('cli_obs').value, arquiteto_nome: document.getElementById('cli_arq_nome').value, arquiteto_whatsapp: document.getElementById('cli_arq_wpp').value, arquiteto_email: document.getElementById('cli_arq_email').value
    };
    try { const response = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const result = await response.json(); if (result.success) { window.location.reload(); } else { alert('Erro: ' + (result.error || 'Erro desconhecido')); } } catch (error) { alert('Erro de rede.'); }
}
...
function toggleOutrosInput() {
    ...
    if (!chkOutros.checked) { inputOutros.value = ''; } else { inputOutros.focus(); }
}
...
function chamarImpressaoFicha(btn) {
    ...
    toggleOutrosInput();
    modalImpressao.classList.remove('hidden'); setTimeout(() => { modalImpressao.classList.remove('opacity-0'); modalImpressaoConteudo.classList.remove('scale-95'); }, 10);
}

function fecharModalImpressao() { modalImpressao.classList.add('opacity-0'); modalImpressaoConteudo.classList.add('scale-95'); setTimeout(() => { modalImpressao.classList.add('hidden'); clienteImpressaoAtual = null; }, 300); }
...
function gerarImpressaoFicha(event) {
    ...
    if (inputData) { const partes = inputData.split('-'); dataFormatada = `${partes[2]}/${partes[1]}/${partes[0]}`; }
    const checkboxes = document.querySelectorAll('.chk-ambiente:checked');
    let ambientes = [];
    checkboxes.forEach(cb => { if (cb.id === 'chk_outros') { const extra = document.getElementById('input_outros_texto').value.trim(); ambientes.push(extra ? `Outros (${extra})` : 'Outros'); } else { ambientes.push(cb.value); } });
    const textoAmbientes = ambientes.length > 0 ? ambientes.join(', ') : 'Nenhum ambiente específico marcado.';
    let linhaEnd = c.end ? c.end : ''; if (c.num) linhaEnd += `, Nº ${c.num}`; if (c.qd) linhaEnd += ` (Qd ${c.qd})`; if (c.comp) linhaEnd += ` - ${c.comp}`;
    let linhaBairro = c.bairro ? c.bairro : ''; if (c.cond) linhaBairro += ` - Condomínio ${c.cond}`;
    let linhaCid = c.cid ? c.cid : ''; if (c.cep) linhaCid += ` - CEP: ${c.cep}`;
    ...
    const janelaPrint = window.open('', '_blank', 'width=900,height=800'); janelaPrint.document.write(html); janelaPrint.document.close(); janelaPrint.focus();
    setTimeout(() => { janelaPrint.print(); janelaPrint.close(); fecharModalImpressao(); }, 500);
}
